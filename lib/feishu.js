...
async function diagnoseFeishu() {
  const config = {
    appId: Boolean(process.env.FEISHU_APP_ID),
    appSecret: Boolean(process.env.FEISHU_APP_SECRET),
    appToken: Boolean(process.env.FEISHU_APP_TOKEN),
    tableId: Boolean(process.env.FEISHU_TABLE_ID),
  };
